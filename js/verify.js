...
(function() {
	if (!window.Promise) {
		document.writeln('<script src="https://cdnjs.cloudflare.com/ajax/libs/es6-promise/4.1.1/es6-promise.min.js"><' + '/' +
			'script>');
	}
})();;
(function($, window, document, undefined) {
	...
	function getPictrue(data, baseUrl) {
		return new Promise((resolve, reject) => {
			$.ajax({
				...
				url: baseUrl + "/common/slide/captcha/get",
	...
	function checkPictrue(data, baseUrl) {
		return new Promise((resolve, reject) => {
			$.ajax({
				type: "post",
				contentType: "application/json;charset=UTF-8",
				url: baseUrl + "/common/slide/captcha/check",
				data: JSON.stringify(data),
				success: function(res) {
					resolve(res)
				},
	...
	Slide.prototype = {
		...
		loadDom: function() {
			...
			var panelHtml = '';
			var wrapHtml = '';
			this.lengthPercent = (parseInt(this.setSize.img_width) - parseInt(this.setSize.block_width) - parseInt(this.setSize
				.circle_radius) - parseInt(this.setSize.circle_radius) * 0.8) / (parseInt(this.setSize.img_width) - parseInt(
			...
			this.plusWidth = parseInt(this.setSize.block_width) + parseInt(this.setSize.circle_radius) * 2 - parseInt(this.setSize
				.circle_radius) * 0.2;
			this.plusHeight = parseInt(this.setSize.block_height) + parseInt(this.setSize.circle_radius) * 2 - parseInt(this.setSize
				.circle_radius) * 0.2;
		...
		move: function(e) {
			if (this.status && this.isEnd == false) {
				...
				if (move_block_left >= (this.htmlDoms.bar_area[0].offsetWidth - parseInt(this.setSize.bar_height) + parseInt(
						parseInt(this.setSize.block_width) / 2) - 2)) {
					move_block_left = (this.htmlDoms.bar_area[0].offsetWidth - parseInt(this.setSize.bar_height) + parseInt(
						parseInt(this.setSize.block_width) / 2) - 2);
				}
	...
	Points.prototype = {
				...
				if (_this.num == _this.options.checkNum) {
					...
					setTimeout(() => {
						let data = {
							...
							"pointJson": _this.secretKey ? aesEncrypt(JSON.stringify(_this.checkPosArr), _this.secretKey) : JSON.stringify(
								_this.checkPosArr),
							"token": _this.backToken,
							phone: _this.options.phone
						}
						var captchaVerification = _this.secretKey ? aesEncrypt(_this.backToken + '---' + JSON.stringify(_this.checkPosArr),
							_this.secretKey) : _this.backToken + '---' + JSON.stringify(_this.checkPosArr)
		...
		refresh: function() {
			...
			getPictrue({
				captchaType: "clickWord",
				phone: this.options.phone
			}, _this.options.baseUrl).then(res => {
				if (res.repCode == "0000") {
